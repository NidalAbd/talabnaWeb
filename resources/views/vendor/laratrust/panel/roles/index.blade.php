    ...
    <div class="-my-2 py-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
      <div class="mt-4 align-middle inline-block min-w-full shadow overflow-hidden sm:rounded-lg border-b border-gray-200">
        <table class="min-w-full">
          <thead>
            <tr>
              <th class="th">{{ __('vendor\laratrust\panel\roles\index.id') }}</th>
              <th class="th">{{ __('vendor\laratrust\panel\roles\index.display_name') }}</th>
              <th class="th">{{ __('vendor\laratrust\panel\roles\index.name') }}</th>
              <th class="th">{{ __('vendor\laratrust\panel\roles\index._permissions') }}</th>
              <th class="th"></th>
            </tr>
          </thead>
          <tbody class="bg-white">
            ...
            <tr>
              ...
              <td class="flex justify-end px-6 py-4 whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                ...
                <a href="{{route('laratrust.roles.edit', $role->field</a>
                @else
                <a href="{{route('laratrust.roles.show', $role->field</a>
                @endif
                <form
                ...
                >
                  ...
                  >{{ __('vendor\laratrust\panel\roles\index.delete') }}</button>
                </form>
              </td>
            </tr>
            @endforeach
