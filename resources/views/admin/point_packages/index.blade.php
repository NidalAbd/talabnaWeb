...
@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            ...
            <p class="text-muted mb-0">{{('admin\point_packages\index.manage_point_packages_and_pricing_plans_') }}</p>
        </div>
...
<div class="container-fluid">
    ...
    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show">
            <button type="button" class="close" data-dismiss="alert" aria-hidden="true">{{('admin\point_packages\index._') }}</button>
    ...
    <div class="row mb-4">
        <div class="col-lg-3 col-6">
            <div class="info-box bg-gradient-info shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">{{('admin\point_packages\index.total_packages') }}</span>
                    <span class="info-box-number">{{ $packages->id</span> }}
        ...
        <div class="col-lg-3 col-6">
            <div class="info-box bg-gradient-success shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">{{('admin\point_packages\index.active_packages') }}</span>
                    <span class="info-box-number">{{ $packages->id</span> }}
                    <div class="progress"><div class="progress-bar bg-light" style="width: {{ $packages->id</div></div> }}
                    <span class="progress-description">
        ...
        <div class="col-lg-3 col-6">
            <div class="info-box bg-gradient-warning shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">{{('admin\point_packages\index.popular_packages') }}</span>
                    <span class="info-box-number">{{ $packages->id</span> }}
                    <div class="progress"><div class="progress-bar bg-light" style="width: {{ $packages->id</div></div> }}
                    <span class="progress-description">
        ...
        <div class="col-lg-3 col-6">
            <div class="info-box bg-gradient-danger shadow-sm">
                ...
                <div class="info-box-content">
                    <span class="info-box-text">{{('admin\point_packages\index.inactive_packages') }}</span>
                    <span class="info-box-number">{{ $packages->id</span> }}
                    <div class="progress"><div class="progress-bar bg-light" style="width: {{ $packages->id</div></div> }}
                    <span class="progress-description">
    ...
    <div class="card card-outline card-primary shadow-sm mb-4">
        <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
            <div class="card-title mb-2 mb-md-0">
                ...
                <span class="badge badge-primary ml-2">{{ $packages->id</span> }}
            </div>
        ...
        <div class="card-body table-responsive p-0">
            <table class="table table-hover table-striped table-bordered align-middle" id="packages-table">
                <thead class="thead-light">
                    <tr>
                        <th width="50">{{('admin\point_packages\index._') }}</th>
                        <th width="80">{{('admin\point_packages\index.icon') }}</th>
                        <th>{{('admin\point_packages\index.name_ar_') }}</th>
                        <th>{{('admin\point_packages\index.name_en_') }}</th>
                        <th>{{('admin\point_packages\index.points') }}</th>
                        <th>{{('admin\point_packages\index.price') }}</th>
                        <th>{{('admin\point_packages\index.discount') }}</th>
                        <th>{{('admin\point_packages\index.duration') }}</th>
                        <th>{{('admin\point_packages\index.status') }}</th>
                        <th width="180">{{('admin\point_packages\index.actions') }}</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse($packages as $package)
                        <tr data-package-id="{{ $package->count() }}">
                            <td>
                                <span class="badge badge-secondary">{{ $package->id</span> }}
                            </td>
                            <td>
                                @if($package->icon)
                                    <i class="{{ $package->icon }}" style="font-size: 18px; color: {{ $package->id</i> }}
                                @else
                                    <i class="fas fa-gift" style="font-size: 18px; color: {{ $package->id</i> }}
                                @endif
                            </td>
                            <td>
                                <strong>{{ $package->id</strong> }}
                                @if($package->is_popular)
                                    <span class="badge badge-warning ml-1">{{('admin\point_packages\index.popular') }}</span>
                                @endif
                            </td>
                            <td>{{ $package->id</td> }}
                            <td>
                                <span class="badge badge-primary">{{ number_format($package->id</span> }}
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                @else
                                    <span class="badge badge-secondary">{{('admin\point_packages\index.no_discount') }}</span>
                                @endif
                            </td>
                            <td>
                                <span class="badge badge-info">{{ $package->id</span> }}
                            </td>
                            <td>
                                @if($package->is_active)
                                    <span class="badge badge-success">{{('admin\point_packages\index.active') }}</span>
                                @else
                                    <span class="badge badge-danger">{{('admin\point_packages\index.inactive') }}</span>
                                @endif
                                @if($package->is_popular)
                                    <span class="badge badge-warning ml-1">{{('admin\point_packages\index.popular') }}</span>
                                @endif
                            ...
                            <td>
                                <div class="btn-group btn-group-sm" role="group">
                                    <a href="{{ route('admin.point_packages.edit', $package->count()) }}" 
                                       class="btn btn-outline-primary" title="Edit Package">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <a href="{{ route('admin.point_packages.show', $package->count()) }}" 
                                       class="btn btn-outline-secondary" title="View Details">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    <button type="button" 
                                            class="btn btn-outline-{{ $package->is_popular ? 'warning' : 'info' }}" 
                                            onclick="togglePopular({{ $package->count() }})"
                                            title="{{ $package->is_popular ? 'Remove Popular' : 'Set Popular' }}">
                                        <i class="fas fa-star"></i>
                                    </button>
                                    <button type="button" 
                                            class="btn btn-outline-success" 
                                            onclick="duplicatePackage({{ $package->count() }})"
                                            title="Duplicate Package">
                                        <i class="fas fa-copy"></i>
                                    </button>
                                    <form action="{{ route('admin.point_packages.destroy', $package->count()) }}" 
                        ...
                        <tr>
                            <td colspan="10" class="text-center py-5">
                                <div class="text-muted">
                                    ...
                                    <h4 class="text-muted">{{('admin\point_packages\index.no_packages_found') }}</h4>
                                    <p class="text-muted mb-4">{{('admin\point_packages\index.create_your_first_point_package_to_get_s') }}</p>
...
function setPopular() {
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success) {
                let packagesHtml = '<div class="form-group">{{('admin\point_packages\index._packageshtml_') }}<label>{{('admin\point_packages\index.select_packages_to_mark_as_popular_') }}</label>{{('admin\point_packages\index._packageshtml_') }}<div class="mt-2" style="max-height: 200px; overflow-y: auto;">{{('admin\point_packages\index._resp') }}<div class="custom-control custom-checkbox">{{('admin\point_packages\index._packageshtml_') }}<input type="checkbox" class="custom-control-input" id="package_' + package.id + '" value="' + package.id + '">{{('admin\point_packages\index._packageshtml_') }}<label class="custom-control-label" for="package_' + package.id + '">{{('admin\point_packages\index._packageshtml_p') }}</label>{{('admin\point_packages\index._packageshtml_') }}</div>{{('admin\point_packages\index._') }}</div></div>';
...
function duplicatePackage(packageId = null) {
    if (packageId) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    let selectHtml = '<select class="form-control" id="packageSelect">{{('admin\point_packages\index._selecthtml_') }}<option value="">{{('admin\point_packages\index.select_a_package_') }}</option>{{('admin\point_packages\index._') }}<option value="' + package.id + '">{{('admin\point_packages\index._package_name_en_package_poi') }}</option>{{('admin\point_packages\index._') }}</select>';
...
function updateTableData(packages) {
    ...
    packages.forEach(function(package) {
        var row = [
            '<span class="badge badge-secondary">{{('admin\point_packages\index._package_id_') }}</span>{{('admin\point_packages\index._package_icon_') }}<i class="' + package.icon + '" style="font-size: 18px; color: ' + (package.color || '#007bff') + ';"></i>{{('admin\point_packages\index._') }}<i class="fas fa-gift" style="font-size: 18px; color: ' + (package.color || '#007bff') + ';"></i>{{('admin\point_packages\index._') }}<strong>{{('admin\point_packages\index._package_name_ar_n_a_') }}</strong>{{('admin\point_packages\index._package_is_popular_') }}<span class="badge badge-warning ml-1">{{('admin\point_packages\index.popular') }}</span>{{('admin\point_packages\index._package_name_en_') }}<span class="badge badge-primary">{{('admin\point_packages\index._package_points_tolocalestring_') }}</span>{{('admin\point_packages\index._') }}<span class="badge badge-success">{{('admin\point_packages\index._package_price_package_curren') }}</span>{{('admin\point_packages\index._package_discount_percenta') }}<span class="badge badge-warning">{{('admin\point_packages\index._package_discount_percentage_') }}</span>{{('admin\point_packages\index._') }}<span class="badge badge-secondary">{{('admin\point_packages\index.no_discount') }}</span>{{('admin\point_packages\index._') }}<span class="badge badge-info">{{('admin\point_packages\index._package_duration_days_days') }}</span>{{('admin\point_packages\index._package_is_active_') }}<span class="badge badge-success">{{('admin\point_packages\index.active') }}</span>{{('admin\point_packages\index._') }}<span class="badge badge-danger">{{('admin\point_packages\index.inactive') }}</span>{{('admin\point_packages\index._package_is_popular_') }}<span class="badge badge-warning ml-1">{{('admin\point_packages\index.popular') }}</span>{{('admin\point_packages\index._generateactionbutto') }}<div class="btn-group btn-group-sm" role="group">{{('admin\point_packages\index._') }}<a href="{{ route("admin.point_packages.edit", "") }}/' + package.id + '" class="btn btn-outline-primary" title="Edit Package">{{('admin\point_packages\index._') }}<i class="fas fa-edit"></i></a>{{('admin\point_packages\index._') }}<a href="{{ route("admin.point_packages.show", "") }}/' + package.id + '" class="btn btn-outline-secondary" title="View Details">{{('admin\point_packages\index._') }}<i class="fas fa-eye"></i></a>{{('admin\point_packages\index._') }}<button type="button" class="btn btn-outline-' + (package.is_popular ? 'warning' : 'info') + '" ' +
           'onclick="togglePopular(' + package.id + ')" title="' + (package.is_popular ? 'Remove Popular' : 'Set Popular') + '">{{('admin\point_packages\index._') }}<i class="fas fa-star"></i></button>{{('admin\point_packages\index._') }}<button type="button" class="btn btn-outline-success" onclick="duplicatePackage(' + package.id + ')" title="Duplicate Package">{{('admin\point_packages\index._') }}<i class="fas fa-copy"></i></button>{{('admin\point_packages\index._') }}<form action="{{ route("admin.point_packages.destroy", "") }}/' + package.id + '" method="POST" class="d-inline ajax-form" ' +
           'data-confirm="Are you sure you want to delete this package?" data-success-message="Package deleted successfully!" data-row-id="' + package.id + '">{{('admin\point_packages\index._csrf_method_delete_') }}<button type="submit" class="btn btn-outline-danger" title="Delete Package"><i class="fas fa-trash"></i></button>{{('admin\point_packages\index._') }}</form></div>';
}
